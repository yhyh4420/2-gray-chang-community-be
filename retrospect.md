...
테스트코드에 대한 고민은 이전부터 있었다. TDD가 중요하다는 사실은 이전부터 알았지만, 이를 적용하는 것은 굉장히 힘들다. 무엇이 제일 힘든가 에 대한 대답은 아마 다음과 같을 것이다.
...
[테스트코드 작성 결과](htmlReport/index.html)
앞서 언급한 대로 브랜치 커버리지 70%, 라인 커버리지 80% 이상을 목표를 정하고 테스트코드를 작성했다.<br>
가장 중요한 서비스 클래스와 컨트롤러 클래스에 대해 집중적으로 테스트했으며, 해당 클래스는 라인 커버리지를 90%이상 달성했다.<br>
테스트코드를 작성하면서, 테스트코드에 대해 먼저 고민한 배달의 민족에서 언급한 부정지표라는 말이 자꾸 떠올랐다.

테스트코드를 작성하는 중 전반적인 코드 리팩토링을 진행했었다. 특히 하드코딩되어있는 ```ResponseEntity```를 유틸함수로 위임하는 등, 리턴값 자체가 달라질 수 있는 상당히 큰 리팩토링이었다.<br>
유틸함수를 작성하면서 가장 큰 도움이 된 부분이 테스트코드였다. 원래 하드코딩되어있는 부분에 맞추어 테스트코드를 작성했고, 리턴값은 변화가 없어야 하므로 어떤 변화가 있는지 테스트코드를 통해 알 수 있었다.<br>
비록 테스트코드를 통과한다는 사실이 높은 품질의 코드임을 보장하지는 못하지만 에러가 발생하지 않는 코드임을 알 수 있고, 나아가 나쁜 품질의 코드가 아님을 보장할 수 있다는 것을 알았다.  
